...
wynik_ztnegbin <- maxLik(logLik = ztnegbin_ll, 
                         grad = ztnegbin_ll_grad, hess = ztnegbin_ll_hess,
                      data = dt2019_1,
                      method = "BFGS",
                      start = c(alpha = unname(coef(m_start)[1] + 1) ,
                                beta = unname(coef(m_start)[2]), 
...
```{r}
list(wynik_lichun,wynik_poisson, wynik_tpoisson, wynik_t2poisson, wynik_negbin, wynik_ztnegbin) %>%
  set_names(c("lichun", "poisson", "tpoisson", "t2poisson", "negbin", "ztnegbin")) %>%
