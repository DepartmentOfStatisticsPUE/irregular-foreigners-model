...
```{r}
udsc2019 <- read_excel("~/git/zbiory/cudzoziemcy/UdSC/2019-roczny.xlsx", sheet = "POB.CZASOWY-DECYZJE",
                        range = cell_cols(1:7), col_names = F) %>%
  slice(6:156) %>%
  select(country = 1, `1` = 5, `2` = 6) %>%
  mutate(`1` = as.numeric(`1`), `2` = as.numeric(`2`),
         country = tolower(country)) %>%
  gather(sex, udsc, -country) %>%
...
```{r}
udsc2019 %>%
  left_join(dict_full %>% distinct(country  = name, country_iso3n=iso3n)) %>%
  filter(udsc > 0) -> udsc_prepared
```
...
pesel_prepared %>%
  ...
  full_join(
    udsc_prepared %>% select(-country)
  ) %>%
  select(half, age, sex, country_iso3n, pesel, border, udsc, matches("police")) %>%
  ...
  filter(is.na(eu)) %>%
  arrange(country, country_iso3n, half, age, sex, udsc, pesel, border, police_nonreg, police_reg, police_all) %>%
  select(country, country_iso3n, half, age, sex, udsc,pesel, border, police_nonreg, police_reg, police_all) %>%
  filter(pesel >0, !country %in% c("Switzerland", "Norway", "United States of America", NA)) -> countries_all
...
```{r}
countries_all %>%
  group_by(country, country_iso3n, half, sex) %>%
  summarise_at(vars(udsc,pesel:police_all), sum) %>%
  mutate(udsc = udsc / 2, 
         country = ifelse(border > 0 & udsc > 0 & udsc / pesel < 1, country, "other")) %>%
  group_by(country, half, sex) %>%
  summarise_at(vars(udsc,pesel:police_all), sum) %>%
  ungroup() %>%
  mutate(border = ifelse(border == 0 & country == "other", 1, border), 
         log_police_pesel = log(udsc/pesel),
         log_border_pesel = log(border/pesel),
         log_pesel = log(pesel)) -> m
...
ggplot(data = ., 
       aes(x = log_police_pesel, y =log_border_pesel, size = sqrt(pesel))) + 
  geom_point(shape = 1) + geom_smooth(method = MASS::rlm, se = F) + facet_grid(~half) + 
  ggpubr::stat_cor(method = "pearson")

m %>%
ggplot(data = ., 
       aes(x = log_pesel,y = log_border_pesel, size = sqrt(pesel), color = factor(sex))) + 
  geom_point(shape = 1) + geom_smooth(method =  MASS::rlm, se = F) + facet_grid(~half) + 
...
m  %>%
  group_by(half) %>%
  do(m = broom::tidy(lm(log_border_pesel ~ -1 + log_pesel + log_police_pesel, data = .)),
     inf = broom::glance(lm(log_border_pesel ~ -1 + log_pesel + log_police_pesel, data = .))) %>%
  unnest(inf) %>%
  unnest(m, names_repair = "universal")  %>%
  dplyr::select(half, estimate, std.error, adj.r.squared)
```
