...
map_df(1:4, 
  ...
  fill(id, age, sex, country, year, quarter),
...
```{r}
police %>%
  filter(age >= 18, sheet != "searching", sex != "Nieznana") %>%
  ...
  distinct(id, sex, age, registered, country_name, half) %>%
...
pesel_prepared %>%
  ...
  select(country, country_iso3n, half, age, sex, pesel, border, police_nonreg, police_reg, police_all) %>%
...
countries_all %>%
  ...
  summarise_at(vars(pesel:police_all), sum) %>%
  ungroup() %>%
  mutate(log_police_pesel = log(police_all/pesel),
...
ggplot(data = ., 
  ...
  geom_point(shape = 1) + geom_smooth(method = "lm", se = F) + facet_wrap(~half) + 
   ggpubr::stat_cor(method = "pearson")

m %>%
 # filter(country != "Ukraine") %>%
ggplot(data = ., 
       aes(x = log_pesel,y = log_border_pesel, size = sqrt(pesel))) + 
  geom_point(shape = 1) + geom_smooth(method = "lm", se = F) + facet_wrap(~half) + 
   ggpubr::stat_cor(method = "pearson")
```
